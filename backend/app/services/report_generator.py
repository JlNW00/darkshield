...
def generate_report_html(audit_data: dict) -> str:
    ...
    target_url = html.escape(audit_data.get("target_url", "Unknown"))
    ...
    for i, p in enumerate(all_patterns, 1):
        ...
        desc = html.escape(c.get("description", p.get("description", "")))
        evidence = html.escape(p.get("evidence", ""))
        confidence = c.get("confidence", p.get("confidence", 0))
        explanation = html.escape(c.get("explanation", ""))
        oecd = html.escape(c.get("oecd_reference", ""))
        remediation = html.escape(c.get("remediation", ""))
        ...
        <div class="pattern-card">
            <div class="pattern-header-row">
                ...
                <span class="sev-badge" style="background:{SEVERITY_COLORS.get(sev, '#888')}">{html.escape(sev.upper())}</span>
                <span class="cat-label">{html.escape(CATEGORY_LABELS.get(cat, cat))}</span>
                <span class="scenario-label">{html.escape(p.get('_scenario', '').replace('_', ' ').title())}</span>
            ...
            {f'<div class="reasoning"><strong>AI Reasoning:</strong> {explanation}</div>' if explanation else ''}
            {oecd_html}
            {remediation_html}
        </div>
        """
    ...
    for cat, count in sorted(cat_counts.items(), key=lambda x: -x[1]):
        ...
        cat_rows += f"""
        <tr>
            <td>{html.escape(CATEGORY_LABELS.get(cat, cat))}</td>
            <td>{count}</td>
            <td>
    ...
    for s in scenarios:
        ...
        scenario_rows += f"""
        <tr>
            <td>{html.escape(s.get('scenario_name', '').replace('_', ' ').title())}</td>
            <td>{pcount}</td>
